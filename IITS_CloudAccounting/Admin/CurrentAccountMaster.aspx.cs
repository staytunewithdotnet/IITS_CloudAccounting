...
namespace IITS_CloudAccounting.Admin
{
  public class CurrentAccountMaster : Page
  {
    ...
    {
      this.Response.Redirect("~/Admin/CurrentAccountMaster.aspx?cmd=view&ID=" + this.gvCurrentAccount.SelectedRow.Cells[0].Text);
      this.BindGrid();
    }
    ...
    {
      this.Response.Redirect("~/Admin/CurrentAccountMaster.aspx?cmd=add");
    }
      ...
      if (this.txtName.Text.Trim().Length > 0)
      {
        ...
        if (!this.checkInDB)
        {
          ...
          if (num != 0)
          {
            ...
            this.Response.Redirect("~/Admin/CurrentAccountMaster.aspx?cmd=view&ID=" + (object) num);
          }
          else
          {
        ...
        if (this.Page.IsValid)
        {
          if (this.txtName.Text.Trim().Length > 0)
          {
            if (this.objCurrentAccountMasterBll.UpdateCurrentAccount(int.Parse(this.hfCurrentAccount.Value.Trim()), this.txtName.Text.Trim(), this.txtDesc.Text.Trim(), this.chkStatus.Checked))
            {
              ...
              this.Response.Redirect("~/Admin/CurrentAccountMaster.aspx?cmd=view&ID=" + this.Request.QueryString["ID"]);
            }
            else
    ...
    {
      this.Response.Redirect("~/Admin/CurrentAccountMaster.aspx?cmd=add&ID=" + this.Request.QueryString["ID"]);
    }
    ...
    protected void btnDelete_Click(object sender, EventArgs e)
    {
      if (this.hfCurrentAccount.Value != null)
      {
        if (this.objCurrentAccountMasterBll.DeleteCurrentAccount(int.Parse(this.hfCurrentAccount.Value)))
        {
          ...
          this.Response.Redirect("~/Admin/CurrentAccountMaster.aspx");
        }
        else
