...
// Type: IITS_CloudAccounting.Admin.BussinessMaster
...
{
  public class BussinessMaster : Page
  {
    ...
    protected GridView gvBussiness;
    protected Button btnAddBussiness;
    protected HiddenField hfBussiness;
    protected HiddenField hfMasterAdminID;
    protected HiddenField hfCompanyID;
    protected HiddenField hfCompanyLoginID;
    protected ObjectDataSource objdsBussiness;
    ...
    {
      this.gvBussiness.DataBind();
    }

    protected override void Render(HtmlTextWriter writer)
    {
      foreach (GridViewRow gridViewRow in this.gvBussiness.Rows)
      {
        if (gridViewRow.RowType == DataControlRowType.DataRow)
        {
          ...
          if (gridViewRow.RowIndex % 2 == 0)
            ...
            gridViewRow.Attributes["onmouseout"] = "this.style.textDecoration='none';this.style.backgroundColor='#fff'";
          gridViewRow.Attributes["onclick"] = this.Page.ClientScript.GetPostBackClientHyperlink((Control) this.gvBussiness, "Select$" + (object) gridViewRow.RowIndex, true);
        }
      }
      base.Render(writer);
    }
    ...
    {
      this.Response.Redirect("~/Admin/BussinessMaster.aspx?cmd=view&ID=" + this.gvBussiness.SelectedRow.Cells[0].Text);
      this.BindGrid();
    }

    protected void gvBussiness_PageIndexChanging(object sender, GridViewPageEventArgs e)
    {
      this.gvBussiness.PageIndex = e.NewPageIndex;
      this.BindGrid();
    }

    protected void btnAddBussiness_Click(object sender, EventArgs e)
    {
      this.Response.Redirect("~/Admin/BussinessMaster.aspx?cmd=add");
    }
      ...
      if (this.txtName.Text.Trim().Length > 0)
      {
        ...
        if (!this.checkInDB)
        {
          ...
          if (num != 0)
          {
            ...
            this.Response.Redirect("~/Admin/BussinessMaster.aspx?cmd=view&ID=" + (object) num);
          }
          else
          {
    ...
    protected void btnUpdate_Click(object sender, EventArgs e)
    {
      try
      {
        if (this.Page.IsValid)
        {
          if (this.txtName.Text.Trim().Length > 0)
          {
            if (this.objBussinessMasterBll.UpdateBussiness(int.Parse(this.hfBussiness.Value.Trim()), this.txtName.Text.Trim(), this.txtDesc.Text.Trim(), this.chkStatus.Checked))
            {
              this.DisplayAlert("Update Successfully..");
              this.Response.Redirect("~/Admin/BussinessMaster.aspx?cmd=view&ID=" + this.Request.QueryString["ID"]);
            }
            else
    ...
    {
      this.Response.Redirect("~/Admin/BussinessMaster.aspx?cmd=add&ID=" + this.Request.QueryString["ID"]);
    }

    protected void btnDelete_Click(object sender, EventArgs e)
    {
      if (this.hfBussiness.Value != null)
      {
        if (this.objBussinessMasterBll.DeleteBussiness(int.Parse(this.hfBussiness.Value)))
        {
          this.DisplayAlert("Details has been Deleted");
          this.Response.Redirect("~/Admin/BussinessMaster.aspx");
        }
        else
