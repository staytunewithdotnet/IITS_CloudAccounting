...
// Type: IITS_CloudAccounting.Admin.TasksInvoicedReport
...
{
  public class TasksInvoicedReport : Page
  {
    ...
    private readonly ReportTasksInvoicedBLL objReportTasksInvoicedBll = new ReportTasksInvoicedBLL();
    private CloudAccountDA.ReportTasksInvoicedDataTable objReportTasksInvoicedDT = new CloudAccountDA.ReportTasksInvoicedDataTable();
    ...
    protected DropDownList ddlTasks;
    protected SqlDataSource sqldsTasks;
    ...
    private void BindGrid()
    {
      ...
      if (this.ddlTasks.SelectedIndex > 0)
      {
        int iTaskID = int.Parse(this.ddlTasks.SelectedItem.Value);
        ...
        this.objReportTasksInvoicedDT = this.objReportTasksInvoicedBll.GetData(iTaskID, sStatus, dtFromDate, dtToDate);
        Label label1 = new Label()
        {
          Text = this.ddlTasks.SelectedItem.Text
        };
        ...
        gridView1.DataSource = (object) this.objReportTasksInvoicedDT;
      ...
      else
      {
        ...
        foreach (ListItem listItem in this.ddlTasks.Items)
        {
          string s = listItem.Value;
          if (!string.IsNullOrEmpty(s))
          {
            int iTaskID = int.Parse(s);
            string sStatus = (string) null;
            DateTime? dtFromDate = new DateTime?();
            DateTime? dtToDate = new DateTime?();
            if (this.ddlStatusSearch.SelectedIndex > 0)
