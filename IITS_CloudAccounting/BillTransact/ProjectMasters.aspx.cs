...
// Type: IITS_CloudAccounting.Admin.ProjectMasters
...
{
  public class ProjectMasters : Page
  {
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
      ...
      if (user != null)
      {
        ...
        if (Roles.IsUserInRole(str, "Admin"))
        {
          ...
          {
            this.Response.Redirect("~/BillTransact/ProjectMaster.aspx");
    ...
    private void ViewRecord(string iD)
    {
      ...
      if (this.objProjectMasterDT.Rows.Count <= 0)
        return;
      this.hfProjectID.Value = this.objProjectMasterDT.Rows[0]["ProjectID"].ToString();
      this.aMethod.Attributes.Add("href", "ProjectMasters.aspx?cmd=add&ID=" + this.hfProjectID.Value);
      this.hfCompanyID.Value = this.objProjectMasterDT.Rows[0]["CompanyID"].ToString();
    ...
    {
      this.Response.Redirect("~/BillTransact/ProjectMasters.aspx?cmd=add&ID=" + ((WebControl) this.gvProject.SelectedRow.Cells[0].FindControl("chkID")).ToolTip);
      this.BindGrid();
    }
    ...
    {
      this.Response.Redirect("~/BillTransact/ProjectMasters.aspx?cmd=add");
    }
          ...
          if (this.gvUsers.Rows.Count > 0)
          {
            for (int index = 0; index < this.gvUsers.Rows.Count; ++index)
            {
              ...
              if (checkBox.Checked)
              {
                ...
                string s2 = checkBox.ToolTip.Replace("Con", "").Replace("C", "");
                switch (cssClass)
                {
                  case "Staff":
                    this.objStaffProjectDetailBll.AddStaffProjectDetail(int.Parse(this.hfCompanyID.Value), int.Parse(s2), iProjectID);
                    continue;
                  case "Contractor":
                    this.objContractorProjectDetailBll.AddContractorProjectDetail(int.Parse(this.hfCompanyID.Value), int.Parse(s2), iProjectID);
                    continue;
                  case "Admin":
    ...
    protected void btnUpdate_Click(object sender, EventArgs e)
    {
      ...
      if (this.txtProjectName.Text.Trim().Length > 0)
      {
        ...
        if (this.objProjectMasterBll.UpdateProject(int.Parse(this.hfProjectID.Value), int.Parse(this.hfCompanyID.Value), this.txtProjectName.Text.Trim(), this.txtProjectDesc.Text.Trim(), new int?(int.Parse(this.ddlClient.SelectedItem.Value)), this.chkCanClient.Checked, new int?(int.Parse(this.ddlManager.SelectedItem.Value)), this.ddlMethod.SelectedItem.Value, dPackageRate, dFlatAmount, new Decimal?(Decimal.Parse(this.txtTimeEstimate.Text.Trim())), true, false, false, false))
        {
          ...
          this.DisplayAlert("Project Successfully Added..!");
          this.Response.Redirect("~/BillTransact/ProjectMasters.aspx?cmd=add&ID=" + (object) int.Parse(this.hfProjectID.Value));
        }
        else
          this.DisplayAlert("Some Error Occured Please Try After Sometime..!");
      }
    ...
    protected void btnArchived_Click(object sender, EventArgs e)
    {
      ...
      if (num == 0)
        ...
        this.Response.Redirect("ProjectMasters.aspx");
    }
    ...
    protected void btnDelete_Click(object sender, EventArgs e)
    {
      ...
      if (num == 0)
        ...
        this.Response.Redirect("ProjectMasters.aspx?de=true&ac=false");
    }
    ...
    protected void btnUnArchived_Click(object sender, EventArgs e)
    {
      ...
      if (num == 0)
        ...
        this.Response.Redirect("ProjectMasters.aspx?ar=true&ac=false");
    }
    ...
    {
      this.Response.Redirect("~/BillTransact/ProjectMasters.aspx?cmd=add&ID=" + ((LinkButton) sender).CommandArgument);
    }
    ...
    protected string GetClientName(string cId)
    {
      if (string.IsNullOrEmpty(cId))
        return "";
      return cId == "0" ? "- internal -" : "---";
    }

    protected void btnEdit_Click(object sender, EventArgs e)
    {
      this.Response.Redirect("ProjectMasters.aspx?cmd=add&ID=" + this.hfProjectID.Value);
    }
