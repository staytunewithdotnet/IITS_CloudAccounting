...
namespace IITS_CloudAccounting.Admin
{
  public class FAQCategoryMaster : Page
  {
    ...
    {
      this.Response.Redirect("~/BillTransact/FAQCategoryMaster.aspx?cmd=view&ID=" + this.gvFAQCategory.SelectedRow.Cells[0].Text);
      this.BindGrid();
    }
    ...
    {
      this.Response.Redirect("~/BillTransact/FAQCategoryMaster.aspx?cmd=add");
    }
        ...
        if (this.Page.IsValid)
        {
          if (this.txtFAQCategoryName.Text.Trim().Length > 0)
          {
            if (this.objFAQCategoryMasterBll.UpdateFAQCategory(int.Parse(this.hfFAQCategory.Value.Trim()), this.txtFAQCategoryName.Text.Trim(), this.txtDesc.Text.Trim(), this.chkStatus.Checked))
            {
              ...
              this.Response.Redirect("~/BillTransact/FAQCategoryMaster.aspx?cmd=view&ID=" + this.Request.QueryString["ID"]);
            }
            else
    ...
    {
      this.Response.Redirect("~/BillTransact/FAQCategoryMaster.aspx?cmd=add&ID=" + this.Request.QueryString["ID"]);
    }
    ...
    protected void btnDelete_Click(object sender, EventArgs e)
    {
      if (this.hfFAQCategory.Value != null)
      {
        if (this.objFAQCategoryMasterBll.DeleteFAQCategory(int.Parse(this.hfFAQCategory.Value)))
        {
          ...
          this.Response.Redirect("~/BillTransact/FAQCategoryMaster.aspx");
        }
        else
