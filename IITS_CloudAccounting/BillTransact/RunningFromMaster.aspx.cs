...
namespace IITS_CloudAccounting.Admin
{
  public class RunningFromMaster : Page
  {
    ...
    {
      this.Response.Redirect("~/BillTransact/RunningFromMaster.aspx?cmd=view&ID=" + this.gvRunningFrom.SelectedRow.Cells[0].Text);
      this.BindGrid();
    }
    ...
    {
      this.Response.Redirect("~/BillTransact/RunningFromMaster.aspx?cmd=add");
    }
      ...
      if (this.txtName.Text.Trim().Length > 0)
      {
        ...
        if (!this.checkInDB)
        {
          ...
          if (num != 0)
          {
            ...
            this.Response.Redirect("~/BillTransact/RunningFromMaster.aspx?cmd=view&ID=" + (object) num);
          }
          else
          {
        ...
        if (this.Page.IsValid)
        {
          if (this.txtName.Text.Trim().Length > 0)
          {
            if (this.objRunningFromMasterBll.UpdateRunningFrom(int.Parse(this.hfRunningFrom.Value.Trim()), this.txtName.Text.Trim(), this.txtDesc.Text.Trim(), this.chkStatus.Checked))
            {
              ...
              this.Response.Redirect("~/BillTransact/RunningFromMaster.aspx?cmd=view&ID=" + this.Request.QueryString["ID"]);
            }
            else
    ...
    {
      this.Response.Redirect("~/BillTransact/RunningFromMaster.aspx?cmd=add&ID=" + this.Request.QueryString["ID"]);
    }
    ...
    protected void btnDelete_Click(object sender, EventArgs e)
    {
      if (this.hfRunningFrom.Value != null)
      {
        if (this.objRunningFromMasterBll.DeleteRunningFrom(int.Parse(this.hfRunningFrom.Value)))
        {
          ...
          this.Response.Redirect("~/BillTransact/RunningFromMaster.aspx");
        }
        else
