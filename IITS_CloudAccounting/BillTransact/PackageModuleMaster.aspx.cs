...
namespace IITS_CloudAccounting.Admin
{
  public class PackageModuleMaster : Page
  {
    ...
    {
      this.Response.Redirect("~/BillTransact/PackageModuleMaster.aspx?cmd=view&ID=" + this.gvPackageModule.SelectedRow.Cells[0].Text);
      this.BindGrid();
    }
    ...
    {
      this.Response.Redirect("~/BillTransact/PackageModuleMaster.aspx?cmd=add");
    }
      ...
      if (this.txtName.Text.Trim().Length > 0)
      {
        ...
        if (!this.checkInDB)
        {
          ...
          if (num != 0)
          {
            ...
            this.Response.Redirect("~/BillTransact/PackageModuleMaster.aspx?cmd=view&ID=" + (object) num);
          }
          else
          {
        ...
        if (this.Page.IsValid)
        {
          if (this.txtName.Text.Trim().Length > 0)
          {
            if (this.objPackageModuleMasterBll.UpdatePackageModule(int.Parse(this.hfPackageModule.Value.Trim()), this.txtName.Text.Trim(), this.txtDesc.Text.Trim(), this.chkStatus.Checked))
            {
              ...
              this.Response.Redirect("~/BillTransact/PackageModuleMaster.aspx?cmd=view&ID=" + this.Request.QueryString["ID"]);
            }
            else
    ...
    {
      this.Response.Redirect("~/BillTransact/PackageModuleMaster.aspx?cmd=add&ID=" + this.Request.QueryString["ID"]);
    }
    ...
    protected void btnDelete_Click(object sender, EventArgs e)
    {
      if (this.hfPackageModule.Value != null)
      {
        if (this.objPackageModuleMasterBll.DeletePackageModule(int.Parse(this.hfPackageModule.Value)))
        {
          ...
          this.Response.Redirect("~/BillTransact/PackageModuleMaster.aspx");
        }
        else
